...
export const enrollClassroom = async (req, res, next) => {
    const { instituteId, grade, userId, } = req.body;
    try {
        const institute = await Institute.findOne({ _id: instituteId });
        if (!institute)
            return res.status(404).send("Institute not found");
        if (institute.grades.length == 0)
            return res.status(404).send("Institute has no grades");
        const user = await User.findOne({ _id: userId });
        if (!user)
            return res.status(404).send("User not found");
        if (user.isEnrolled)
            return res.status(400).send("User already enrolled in a grade");
        const modifiedGrade = await institute.grades.find((iteratingGrade) => iteratingGrade.grade == grade);
        if (!modifiedGrade)
...
//# sourceMappingURL=institute-controllers.js.map